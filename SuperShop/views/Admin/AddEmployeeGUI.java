...
public class AddEmployeeGUI extends JFrame implements ActionListener{
   ...
   // Add employeeController as a field
   ...
   public AddEmployeeGUI()
   {
    ...
    // In the constructor, after panel initialization, add a section title label
    ...
    this.userIdField.setBounds(200, 100, 300, 30);
    this.userIdField.setEditable(false);
    this.userIdField.setText(generateNextEmployeeId());
    this.userIdField.setBackground(new Color(245, 245, 245));
    ...
    this.saveButton = new JButton("Add Employee");
    this.saveButton.setBounds(120, 750, 200, 30);
    this.saveButton.setBackground(new Color(34, 139, 34)); // green
    ...
    this.clearButton.setBackground(new Color(255, 140, 0)); // orange
    ...
    this.backButton.setBackground(new Color(70, 130, 180)); // blue
    ...
    // For all JLabels, set font and color
    ...
    // For all JComboBox, set font, background, foreground, and rounded border
    ...
    // After creating all text fields, add Enter key navigation
...
private void saveEmployee() {
    ...
    try {
        ...
        // Save employee using controller
...
private void clearForm() {
    ...
    photoPreviewLabel.setText("No Photo");
    userIdField.setText(generateNextEmployeeId());
}
...
private Color darken(Color color, float fraction) {
   ...
   private String generateNextEmployeeId() {
    ...
    for (Employee emp : employees) {
        if (emp != null && emp.getUserId() != null && emp.getUserId().startsWith("EMP")) {
            try {
                String numberStr = emp.getUserId().substring(3);
...
private void setupEnterKeyNavigation() {
    // Create array of text fields in tab order
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                    if (nextIndex < textFields.length) {
                        ...
                        // After name field, focus on gender combo
    ...
    // Add Enter key listeners for combo boxes and spinner
...
private void choosePhoto() {
    ...
    if (result == JFileChooser.APPROVE_OPTION) {
        ...
        // Use the exact photo path provided by the user
        selectedPhotoPath = selectedFile.getAbsolutePath();
        try {
            // Show preview using the original file path
